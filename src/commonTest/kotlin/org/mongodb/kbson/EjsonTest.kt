...
class EjsonTest {
     ...
     *
     * KSerializer provides access to the serializer in compile time only. This forces to retrieve the serializer when
     * we declare the test data rather than when we use the data in a test.
     */
    private inline fun <reified T> valueWithSerializer(value: T) = value to EJson.serializersModule.serializer<T>()
    ...
    private val supportedKotlinTypesDataset: List<Pair<Any, KSerializer<out Any>>> =
        SupportedKotlinTypes.values().flatMap { type ->
            when (type) {
                SupportedKotlinTypes.BOOLEAN -> listOf(valueWithSerializer(true), valueWithSerializer(false))
                SupportedKotlinTypes.SHORT ->
                    listOf(valueWithSerializer(Short.MIN_VALUE), valueWithSerializer(Short.MAX_VALUE))
                SupportedKotlinTypes.INT ->
                    listOf(valueWithSerializer(Int.MIN_VALUE), valueWithSerializer(Int.MAX_VALUE))
                SupportedKotlinTypes.LONG ->
                    listOf(valueWithSerializer(Long.MIN_VALUE), valueWithSerializer(Long.MAX_VALUE))
                SupportedKotlinTypes.FLOAT ->
                    listOf(valueWithSerializer(Float.MIN_VALUE), valueWithSerializer(Float.MAX_VALUE))
                SupportedKotlinTypes.DOUBLE ->
                    listOf(valueWithSerializer(Double.MIN_VALUE), valueWithSerializer(Double.MAX_VALUE))
                SupportedKotlinTypes.STRING ->
                    listOf(valueWithSerializer("hello world"), valueWithSerializer(""), valueWithSerializer("🚀💎"))
                SupportedKotlinTypes.CHAR ->
                    listOf(valueWithSerializer('4'), valueWithSerializer('c'), valueWithSerializer('['))
                SupportedKotlinTypes.BYTEARRAY ->
                    listOf(
                        valueWithSerializer(byteArrayOf(10, 0, 10)),
                        valueWithSerializer(byteArrayOf()),
                        valueWithSerializer(byteArrayOf(0, 0)))
                SupportedKotlinTypes.ARRAY -> listOf(valueWithSerializer(arrayOf<String>("hello world")))
                SupportedKotlinTypes.LIST ->
                    listOf(
                        valueWithSerializer(listOf<String>("hello world")),
                        valueWithSerializer(listOf<List<String>>(listOf("hello world"))),
                    )
                SupportedKotlinTypes.SET ->
                    listOf(
                        valueWithSerializer(setOf<String>("hello world")),
                        valueWithSerializer(setOf<Set<String>>(setOf("hello world"))),
                    )
                SupportedKotlinTypes.MAP ->
                    listOf(
    ...
    private val bsonDataSet: List<Pair<BsonValue, KSerializer<out BsonValue>>> =
        BsonType.values()
            .flatMap {
                when (it) {
                    ...
                    BsonType.BOOLEAN ->
                        listOf(
                            valueWithSerializer(BsonBoolean.TRUE_VALUE), valueWithSerializer(BsonBoolean.FALSE_VALUE))
                    ...
                    BsonType.DB_POINTER -> listOf(valueWithSerializer(BsonDBPointer("test", BsonObjectId())))
                    BsonType.JAVASCRIPT ->
                        listOf(
                            valueWithSerializer(
                                BsonJavaScript(
            ...
            .let { dataset ->
                ...
                listOf(
                    ...
                    valueWithSerializer(
                        BsonDocument(bsonValues.mapIndexed { index, bsonValue -> "$index" to bsonValue }.toMap())))
                dataset
            }
    ...
    fun roundTripAllTypes() {
        supportedKotlinTypesDataset +
            bsonDataSet.forEach { (data: Any, serializer: KSerializer<out Any>) ->
    ...
    fun userDefinedClasses() {
        val value = AllTypes().apply { allTypesObject = AllTypes() }
    ...
    fun userDefinedClasses_subsetOfAllFields_ignoreUnknownKeys() {
        ...
        assertRoundTrip(value) { expected, actual: SubsetOfAllTypes -> assertEquals(expected.string, actual.string) }
    }
    ...
    fun userDefinedClasses_optionalFields() {
        ...
        assertRoundTrip(value) { expected, actual: OptionalFields -> assertEquals(expected.string, actual.string) }
    }
    ...
    fun userDefinedClasses_notMappedFields() {
        ...
        val encodedValue = EJson.encodeToString(value)
        assertFailsWithMessage<SerializationException>(
            "Could not decode field " + "'unexistent': Undefined value on a non-optional field") {
    ...
    fun userDefinedClasses_wrongFieldType() {
        ...
        assertFailsWithMessage<SerializationException>(
            "Could not decode field" + " 'string': Value expected to be of type BOOLEAN is of unexpected type STRING") {
    ...
    fun userDefinedClasses_contextualClass() {
        val expected = ContextualClassHolder(contextualClass = ContextualClass(string = "helloworld"))

        val contextualSerializer =
            object : KSerializer<ContextualClass> {
                override val descriptor: SerialDescriptor =
                    buildClassSerialDescriptor("ContextualClass") { element("string", String.serializer().descriptor) }
        ...
        assertRoundTrip(
            ...
            ejson =
                EJson(
                    serializersModule =
                        SerializersModule { contextual(ContextualClass::class, contextualSerializer) })) {
            expected,
            actual: ContextualClassHolder ->
    ...
    fun userDefinedClasses_polymorphic() {
        EJson(
                serializersModule =
                    SerializersModule { polymorphic(PolymorphicInterface::class) { subclass(ClassA::class) } })
            .let { ejson ->
    ...
    @Serializable data class ClassA(override val name: String) : PolymorphicInterface
    ...
    @Serializable data class SubsetOfAllTypes(val string: String)

    @Serializable data class TransientFields(val string: String, @Transient val char: Char = '0')

    @Serializable data class OptionalFields(val string: String?)

    @Serializable
    data class OptionalFieldsWithDefaults(val required: String, val optionalWithDefault: String = "Hello, world!")

    @Serializable data class NotMappedFields(val string: String?, val unexistent: String)

    @Serializable data class NotMappedOptionalFields(val string: String?, val unexistent: String? = null)

    @Serializable data class WrongFieldType(val string: Boolean)

    data class ContextualClass(val string: String)

    @Serializable data class ContextualClassHolder(@Contextual val contextualClass: ContextualClass)
