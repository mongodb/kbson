...
val publishSystemsCheck =
    tasks.register("publishSystemsCheck") {
        actions =
            listOf<Action<Task>>(
                Action() {
                    val systemErrorMessage: String? =
                        if (System.getProperty("os.name") != "Mac OS X") {
                            "Publishing requires Mac OS X"
                        } else if (!hasAndroidSDK) {
                            "Publishing requires the Android SDK"
                        } else if (!version.toString().endsWith("-SNAPSHOT") && gitVersion != version) {
                            "Version mismatch: $version != $gitVersion"
                        } else {
                            null
                        }

                    if (systemErrorMessage.isNullOrEmpty()) {
                        println("Publishing: ${project.name} : $version")
                    } else {
                        val cause =
                            """
                | =========================================
...
tasks.register("publishSnapshots") {
    ...
    dependsOn(publishSystemsCheck)
    if (version.toString().endsWith("-SNAPSHOT")) {
        shouldRunAfter(publishSystemsCheck)
